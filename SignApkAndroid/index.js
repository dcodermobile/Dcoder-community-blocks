...
const signApkFile = async (apkFile, signingKeyFile, alias, keyStorePassword, keyPassword) => {
  ...
  child_process.spawnSync(`"${zipAlign}"`, ['-c', '-v', '4', `"${apkFile}"`], { shell: true })

  child_process.spawnSync(`"cp"`, [`"${apkFile}"`, `"${alignedApkFile}"`], { shell: true })
  ...
  // Verify
  child_process.spawnSync(`"${apkSigner}"`, ['verify', `"${signedApkFile}"`], { shell: true })
