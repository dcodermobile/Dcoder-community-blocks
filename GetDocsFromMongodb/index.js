...
const main = async(inputs, auths, event) => {
    ...
    fieldsToBeConvertedInObjectId.split(",").forEach(item =>{
      ...
      if(nestedLevels.length == 1){
        if(Array.isArray(query[nestedLevels[0]])){
          ...
          query[nestedLevels[0]] = new ObjectId(query[nestedLevels[0]])
        ...
        if(Array.isArray(query[nestedLevels[0]][nestedLevels[1]])){
          ...
          query[nestedLevels[0]][nestedLevels[1]] = new ObjectId(query[nestedLevels[0]][nestedLevels[1]][nestedLevels[2]])
        ...
        if(Array.isArray(query[nestedLevels[0]][nestedLevels[1]])){
          ...
          query[nestedLevels[0]][nestedLevels[1]][nestedLevels[2]] = new ObjectId(query[nestedLevels[0]][nestedLevels[1]][nestedLevels[2]])
      }else{
        throw(new Error('More than 3 level of nesting in fields to be converted to ObjectId is not supported.'))
      }
  ...
  let result = await collection.find(query).project(projection).sort(sort).skip(skip).limit(limit).toArray()
  client.close()
  return result
}
